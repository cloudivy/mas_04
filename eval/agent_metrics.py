"""
Agent Metrics & Evaluation — tracks per-agent run statistics.

Implements a simplified Agent Stability Index (ASI) across 4 dimensions:
  1. Response Consistency   — output length variance
  2. Reasoning Stability    — keyword overlap with prior runs
  3. Task Adherence         — task keyword coverage in output
  4. Latency Stability      — elapsed time variance

ASI = weighted mean of the 4 scores (0–100).

Metrics are persisted to .store/{agent_id}_metrics.json.
Use eval/drift_report.py to run cross-agent analysis.
"""
...
class AgentMetrics:
    ...
    def _compute_asi(self, run: dict, prior_runs: list[dict]) -> dict:
        ...
        # 1. Response Consistency (output length stability)
        lengths = [len(r["output"].split()) for r in prior_runs[-5:]] + [len(run["output"].split())]
        if len(lengths) > 1:
            mean_l = sum(lengths) / len(lengths)
            variance = sum((l - mean_l) ** 2 for l in lengths) / len(lengths)
            scores["response_consistency"] = max(0, 100 - min(variance / max(mean_l, 1) * 100, 100))
        else:
            scores["response_consistency"] = 100.0

        # 2. Reasoning Stability (overlap with last run)
        if prior_runs:
        ...
        # 3. Task Adherence (task keywords appear in output)
        task_words = set(re.findall(r"\w{4,}", run["task"].lower()))
        out_words = set(re.findall(r"\w+", run["output"].lower()))
        if task_words:
            scores["task_adherence"] = round(len(task_words & out_words) / len(task_words) * 100, 1)
        else:
        ...
        if len(latencies) > 1:
            ...
            scores["latency_stability"] = max(0, 100 - min(lat_var * 10, 100))
        else:
            scores["latency_stability"] = 100.0

        # ASI composite
        weights = {"response_consistency": 0.3, "reasoning_stability": 0.25,
                   "task_adherence": 0.35, "latency_stability": 0.1}
        asi = sum(scores[k] * w for k, w in weights.items())
        scores["asi"] = round(asi, 1)
        return scores

    def record_run(self, task: str, output: str, context: str,
