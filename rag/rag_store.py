...
Lightweight document store with keyword-based retrieval.
Upgrade path: swap _score() with sentence-transformers embeddings
or plug in ChromaDB / Pinecone for production use.

Usage:
    store = RAGStore()
    store.add_document("doc1", "FastAPI is a modern Python web framework...")
    results = store.retrieve("Python web API", top_k=2)
"""
...
class RAGStore:
    ...
    def _score(self, query: str, text: str) -> float:
        """Keyword overlap score. Replace with embedding similarity for production."""
        q_words = set(re.findall(r"\w+", query.lower()))
        d_words = set(re.findall(r"\w+", text.lower()))
        if not d_words:
            return 0.0
    ...
    def list_documents(self) -> list[dict]:
        return [{"id": d["id"], "title": d["title"], "added_at": d["added_at"]} for d in self._read()]
