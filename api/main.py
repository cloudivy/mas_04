"""
FastAPI Backend for the Multi-Agent Framework.

Routes:
  GET  /                        Health check
  GET  /agents                  List all registered agents
  POST /run                     Run the full pipeline (blocking)
  GET  /run/{run_id}            Get a past run result
  GET  /metrics/{agent_id}      Get per-agent ASI history
  GET  /metrics                 Cross-agent metrics summary
  POST /rag/add                 Add a document to RAG store
  GET  /rag/list                List RAG documents
  DELETE /rag/{doc_id}          Delete a RAG document
  GET  /memory/{agent_id}       Get agent memory contents
...
app = FastAPI(
    ...
    description="Pluggable LLM multi-agent system with memory, RAG, tools, and observability",
    version="1.0.0",
)
...
# In-memory run store (replace with DB for production)
_runs: dict[str, dict] = {}
...
@app.get("/")
def health():
    return {"status": "ok", "service": "multi-agent-framework", "version": "1.0.0"}
